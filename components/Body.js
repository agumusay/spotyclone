		...
		<section className='bg-black ml-24 py-4 space-y-8 md:max-w-6xl flex-grow md:mr-2.5'>
			...
			<div className='grid grid-cols-2 p-4 py-4 overflow-y-scroll scrollbar-hide h-96 lg:grid-cols-3 xl:grid-cols-4 gap-x-4 gap-y-8'>
				{searchResults.length === 0
					? newReleases.slice(0, 4).map(track => <Poster key={track.id} track={track} chooseTrack={chooseTrack} />)
			...
			<div className='absolute flex min-w-full ml-6 gap-x-8 md:relative'>
				...
				<div className='w-full pr-11'>
					<h2 className='mb-3 font-bold text-white'>{searchResults.length === 0 ? 'New Releases' : 'Tracks'}</h2>
